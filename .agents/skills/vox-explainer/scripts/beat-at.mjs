...
function timeOfOffset(lineKey, offset) {
  ...
  const pts = rec.beats.map((b) => {
    const off = body.indexOf(b.text, 0);
    return { off: body.indexOf(b.text, 0) >= 0 ? body.indexOf(b.text, 0) : 0, t: b.start };
  });
...
for (const [lineKey0, phrases] of Object.entries(spec)) {
  ...
  for (const p of phrases) {
    ...
    rows.push({
      ...
      frameAt: t === null ? null : Number(Math.max(0.2, 0.3 + t - lead).toFixed(2)),
      spokenFor: t === null ? null : Number((0.3 + t).toFixed(3)),
    });
  }
