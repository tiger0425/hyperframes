...
| 5    | `build`      | {{FRAMES}} 帧 + `NN-slug.motion.json` + `index.html`（v2 项目另有 `tools/theme.json` / `tools/assemble-table.json` / `ledger.json`） | 门禁链 v2 全绿：`lint` 0/0 · `audit-frames` 0 finding · `sync --check` N/N · `verify-timeline` 0 error · `seam-gate verify` 绿（v2 项目；旧片跳过）· `check` 自证（`samples` / `contrast.checked` / `duration≈根总长` / 0 error + 0 warning，且带 `--out`） |
...
| `hf.mjs`                   | 门禁 CLI 包装（lint/check/snapshot）                                         | `node <skill>/scripts/hf.mjs <lint\|check\|snapshot> [args] [--out <file>]`                                    | `lint: 0 error / 0 warning（自动读取 JSON）; check: samples>0 && contrast>0 && duration≈index.html 根总长（容差 0.1s）&& 0 error（未传 --out 返回 3）` |
...
| `gen-index.mjs`       | **装配 `index.html`**（槽位 + 旁白轨 + 音效轨 + **纸 ASMR 轨 12**，读可选 `tools/asmr.json`）                                                                                                                                                            | `node tools/gen-index.mjs`                                                                                                                           | 槽位数 == 帧数；总长 == `slots.mjs` 的 `TOTAL`；ASMR 完整区间不与旁白窗口重叠，违规阻断生成 |
...
| `synthesize_voice.py` | 旁白合成 + 量真实秒数 + 写 `.media/voice-manifest.json`（支持 `--frame NN` 单条重跑并并回清单；帧数从 `tools/frames-data.mjs`、清单或 `--expected-frames` 动态读取） | `python tools/synthesize_voice.py [--frame NN] [--expected-frames N] [--list]`                                                            | 解析条数与帧号集合 == `frames-data.mjs`；重复帧号拒绝；每条合成成功且 wav 头可读；合计秒数与清单一致 |
| `align-cues.py`       | **词级对齐**：`SCRIPT.md` + `tools/cues.json` + faster-whisper → `tools/cue-times.json`                                                                                                                                                                  | `python tools/align-cues.py --model medium [--frame NN] [--min-align-hit 0.6] [--max-align-distance 3]`                                           | 线索 N/N 命中；整句 `align_hit` ≥ 60%；每个锚点最近邻距离 ≤ 3；打印 `align_hit` 与逐帧 ASR 转写 |
...
由子进程直接写出 JSON，程序化自验证 `samples.length`、`contrast.checked` 均有效，且 `duration` 与 `index.html` 根 `data-duration` 相差不超过 0.1 秒（返回 0；未指定 `--out` 返回退出码 3 说明未自验证）。
